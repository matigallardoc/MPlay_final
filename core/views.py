...
def registrarme(request):
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            perfil.save()
            premium = ' y aprovechar tus descuentos especiales como cliente PREMIUM' #if perfil.success
    ...
    if request.method == 'GET':
        if accion == 'crear':
            ...
            try:
                ...
                messages.error(request,'El producto No fue eliminado correctamente')
                return redirect(productos,'crear','0')
...
def usuarios(request, accion, id):
    ...
    if request.method == 'POST':
        ...
        if form_usuario.is_valid() and form_perfil.is_valid():
            ...
            messages.error(request, 'El usuario no se pudo guardar correctamente')
            show_form_errors(request, [form_usuario, form_perfil])
        ...
        # CREAR: lógica para actualizar los datos del usuari
...
def bodega(request):
    ...
    if request.method == 'POST':
        ...
        if cantidad ==1:
            messages.success(request, f'El producto {producto.nombre} fue agregado a la bodega')
        else:
            messages.success(request, f'Se han agregado {cantidad} de {producto.nombre} a la bodega')
